...
import { ArrowLeft, Phone, Handshake, CalendarCheck, Wallet, Link as LinkIcon, Send, LayoutGrid, Plus, X, Pencil } from 'lucide-react'
...
type Tab = 'overview' | 'deal' | 'meeting' | 'payment'
...
export function FollowupDetailPage() {
  ...
  const { data: leadActivity = [], isLoading: isLoadingActivity } = useLeadActivity(followup?.lead_id)
  ...
  const meetings = allMeetings
    .filter((m) => m.contact_id === followup.contact_id)
    .sort((a, b) => new Date(b.meeting_time).getTime() - new Date(a.meeting_time).getTime())
  const deals = (dealsData?.deals ?? [])
    .filter((d) => d.contact_id === followup.contact_id)
    .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
  const payments = (paymentsData?.payments ?? [])
    .filter((p) => p.contact_id === followup.contact_id)
    .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
  const discussionEntries = leadActivity.filter((a) => a.kind === 'discussion')

  function saveDiscussion() {
    if (!followup.lead_id || !discussionDraft.trim()) return
    updateLead.mutate(
      { id: followup.lead_id, patch: { discussion: discussionDraft } },
      {
        onSuccess: () => {
  ...
  function submitDeal() {
    ...
    createDeal.mutate(
      { contactId: followup.contact_id, dealName: dealName.trim(), dealAmount: Number(dealAmount), dealDetails: dealDetails.trim() || undefined },
      {
        onSuccess: () => {
          ...
          <div className="flex items-center gap-3">
            ...
            {followup.contact_phone && (
              <a
                ...
                aria-label={`Call ${followup.contact_phone}`}
                title={`Call ${followup.contact_phone}`}
